...
# 🔹 Hauptprogramm: Flask und Telegram Polling starten
if __name__ == "__main__":
    # Starte den Flask-Server in einem separaten Thread
    Thread(target=lambda: app.run(host="0.0.0.0", port=PORT)).start()

    # Starte Telegram-Polling mit Fehler-Neustart
    asyncio.run(application.run_polling(allowed_updates=telegram.constants.Update.ALL_TYPES, restart_on_failure=True))
